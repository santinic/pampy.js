...
describe("matchArray", () => {
  it("zipLongest", () => {
    assert.deepEqual(zipLongest([1, 2, 3], [1, 2]), [
      [1, 1],
      [2, 2],
      [3, PAD_VALUE]
    ]);
    assert.deepEqual(zipLongest([1, 2], [1, 2, 3]), [
      [1, 1],
      [2, 2],
      [PAD_VALUE, 3]
    ]);
  ...
  it("values", () => {
    ...
    assert.deepEqual(matchArray([1, STRING, NUMBER], [1, "2", 3]), [
      true,
      ["2", 3]
    ]);
  });
  ...
  it("nested", () => {
    assert.deepEqual(matchArray([1, [_, 3], _], [1, [2, 3], 4]), [
      true,
      [2, 4]
    ]);
  });
...
describe("matchDict", () => {
  ...
  it("dict ordering", () => {
    for (let i = 0; i < 100; i++) {
      assert.deepEqual(matchDict({ a: _, b: _ }, { a: 1, b: 2 }), [
        true,
        [1, 2]
      ]);
    }
  ...
  it("ambiguous double _", () => {
    assert.deepEqual(matchDict({ a: _, _: _ }, { a: 1, b: 2 }), [
      true,
      [1, "b", 2]
    ]);
    assert.deepEqual(matchDict({ a: STRING, _: _ }, { a: "1", b: 2 }), [
      true,
      ["1", "b", 2]
    ]);
  });
...
describe("match", () => {
  ...
  it("lambda args TAIL", () => {
    assert.deepEqual(match([1, 2, 3, 4], [1, _, TAIL], (x, tail) => tail), [
      3,
      4
    ]);
    assert.deepEqual(match([1, 2, 3, 4], [1, _, TAIL], (a, b) => [a, b]), [
      2,
      [3, 4]
    ]);
  });
  ...
  it("fibonacci", () => {
    function fib(n) {
      return match(n, 1, 1, 2, 1, _, x => fib(x - 1) + fib(x - 2));
    }
  ...
  it("lisp", () => {
    function lisp(exp) {
      return match(
        ...
        x => x,
        [Function, REST],
        (f, rest) => f.apply(null, rest.map(lisp)),
        Array,
        l => l.map(lisp),
        _,
        x => x
  ...
  it("len", () => {
    function len(l) {
      return match(l, [], 0, [_, TAIL], (head, tail) => 1 + len(tail));
    }
    assert.equal(len([{}]), 1);
  ...
  it("tree leafs", () => {
    function findLeafs(node) {
      return match(node.childs, [], () => node.name, [TAIL], tail =>
        lodash.flatMap(tail, findLeafs)
      );
    }
    ...
    let tree = {
      ...
      childs: [
        {
          name: 2,
          childs: [{ name: 3, childs: [] }, { name: 4, childs: [] }]
        },
        { name: 5, childs: [] }
      ]
    };
  ...
  it("match null and undefined", () => {
    function f(x) {
      return match(x, null, "null", undefined, "undefined", NUMBER, "number");
    }
...
describe("matchAll", () => {
  it("basic", () => {
    let rows = [{ a: 1, b: 2 }, { a: 3, b: 4 }, { a: 5, b: 6 }];
    assert.deepEqual(matchAll(rows, { a: _, b: _ }, (a, b) => [a, b]), [
      [1, 2],
      [3, 4],
      [5, 6]
    ]);
    assert.deepEqual(
      matchAll(rows, { a: _, _: _ }, (x1, x2, x3) => [x1, x2, x3]),
      [[1, "b", 2], [3, "b", 4], [5, "b", 6]]
    );
  });
  ...
  it("Big json", () => {
    ...
    let avg =
      bigJson.data.children.map(c => c.data.score).reduce((a, b) => a + b) /
      bigJson.data.children.length;
